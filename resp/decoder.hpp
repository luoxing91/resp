...
class decoder {
  ...
  public:
    decoder() : stat_(st_start), bulk_size_(0) {}
  ...
  private:
    ...
    static bool isctrl(int c) { return (c >= 0 && c <= 31) || (c == 127); }
        ...
        for (; i < size; ++i) {
            ...
            case st_string_lf:
                if (c == '\n') {
                    ...
                    value_stack_.push(
                        unique_value(buf_.data(), buf_.size(), ty_string));
            ...
            case st_error_lf:
                if (c == '\n') {
                    ...
                    value_stack_.push(
                        unique_value(buf_.data(), buf_.size(), ty_error));
                    //          return std::make_pair(i + 1, completed);
                    return result(completed, i + 1, get_result());
                } else {
                    stat_ = st_start;
            ...
            case st_bulk_lf:
                if (c == '\n') {
                    ...
                    //          value_stack_.push(buf_);
                    value_stack_.push(
                        unique_value(buf_.data(), buf_.size(), ty_bulkstr));
                    //          return std::make_pair(i + 1, completed);
                    return result(completed, i + 1, get_result());
                } else {
                    stat_ = st_start;
            ...
            case st_array_size_lf:
                if (c == '\n') {
                    ...
                    if (arraySize == -1 || arraySize == 0) {
                        ...
                        if (i + 1 != size) {
                            //              std::pair<size_t, ParseResult>
                            //              parseResult = parseArray(ptr + i +
                            //              1, size - i - 1);
                            result res =
                                decode_array(ptr + i + 1, size - i - 1);
