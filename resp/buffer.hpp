...
class buffer {
  public:
    ...
    buffer(char const *ptr)
        : capacity_(0), data_(const_cast<char *>(ptr)),
    ...
    buffer(std::string const &str)
        : capacity_(0), data_(const_cast<char *>(str.data())),
          size_(str.size()) {}
    ...
    buffer(char const *ptr, size_t size)
        : capacity_(0), data_(const_cast<char *>(ptr)), size_(size) {}

    /// Copy constructor.
    buffer(buffer const &other)
        : capacity_(other.capacity_), data_(0), size_(other.size_) {
        if (other.is_ref()) {
            data_ = other.data_;
  ...
  public:
    ...
    bool is_ref() const { return capacity_ == 0; }
    ...
    bool is_large() const { return capacity_ > RESP_SMALL_BUFFER_SIZE; }
    ...
    void reserve(size_t capacity) {
        if (capacity > capacity_) {
            ...
            capacity = capacity % RESP_SMALL_BUFFER_SIZE == 0
                           ? capacity
                           : RESP_SMALL_BUFFER_SIZE *
                                 (capacity / RESP_SMALL_BUFFER_SIZE + 1);
    ...
    /// Append other buffer's data, if self is ref, then change to small or
    /// large.
