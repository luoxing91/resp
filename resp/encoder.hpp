 ...
 *  #1 copyable
 *  #2 has member method Buffer::append(Buffer const&/char/char const*), like
 * std::string #3 has non-explicit constructors, like std::string
 */
template <typename Buffer> class encoder {
    typedef Buffer buffer_t;
    typedef encoder<buffer_t> self_t;

  public:
    explicit encoder() : buffers_(0) {}
  ...
  public:
     ...
     * @return At least one buffer, may multi, if multi, user could use
     * scatter-gather io to send them.
     */
    ...
    std::vector<buffer_t> encode(buffer_t const &cmd, buffer_t const &arg1) {
        std::vector<buffer_t> buffers;
        buffers.reserve(2);
    ...
    /// Encode redis command + arg(s).
    std::vector<buffer_t> encode(buffer_t const &cmd, buffer_t const &arg1,
                                 buffer_t const &arg2, buffer_t const &arg3) {
        std::vector<buffer_t> buffers;
        buffers.reserve(3);
        append_size(buffers, '*', 3);
        append(buffers, cmd);
    ...
    std::vector<buffer_t> encode(buffer_t const &cmd, buffer_t const &arg1,
                                 buffer_t const &arg2, buffer_t const &arg3,
                                 buffer_t const &arg4, buffer_t const &arg5) {
        std::vector<buffer_t> buffers;
        buffers.reserve(5);
    ...
    /// Encode redis command + arg(s).
    std::vector<buffer_t> encode(buffer_t const &cmd,
                                 std::vector<buffer_t> const &args) {
        std::vector<buffer_t> buffers;
  ...
  public:
    class command {
      public:
        ...
        command(self_t &enc, buffer_t const &name) : enc_(&enc), name_(name) {}

        command(command const &other) : enc_(other.enc_), name_(other.name_) {}
  ...
  public:
    ...
    static void append_size(std::vector<buffer_t> &buffers, char ty,
                            size_t size) {
        if (buffers.empty()) {
    ...
    static void encode(std::vector<buffer_t> &buffers, buffer_t const &cmd,
                       std::vector<buffer_t> const &args) {
    ...
    void queued_buffer(buffer_t const &buf) { cmd_args_.push_back(buf); }

    std::vector<buffer_t> &get_cmd_args() { return cmd_args_; }
