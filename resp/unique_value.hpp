...
/**
 * @note Under C++98/03 there is no move, so just use
 * copy(consturctor/assignment) to instead; after C++11 could use move directly.
 */
class unique_value {
  public:
    unique_value() : ty_(ty_null), integer_(0) {}

    unique_value(unique_value const &other)
        : ty_(other.ty_), integer_(other.integer_), array_(other.array_) {
    ...
    unique_value(char const *str) : ty_(ty_string), integer_(0) {
        str_.append(str);
    }

    unique_value(char const *str, value_type ty) : ty_(ty), integer_(0) {
        assert(ty_ != ty_array);
    ...
    unique_value(int64_t integer) : ty_(ty_integer), integer_(integer) {}

    unique_value(unique_array<unique_value> const &array)
        : ty_(ty_array), integer_(0), array_(array) {}
  ...
  public:
    ...
    value_type type() const { return ty_; }

    /// Check if null.
    operator bool() const { return ty_ != ty_null; }

    /// Check if not null.
    bool operator!() const { return ty_ == ty_null; }
