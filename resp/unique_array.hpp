...
template <typename T> class unique_array {
    class node {
      public:
        node()
            : chunk_((T *)std::malloc(sizeof(T) *
                                      RESP_UNIQUE_ARRAY_NODE_CAPACITY)),
              size_(0) {
            if (chunk_ == 0) {
                throw std::bad_alloc();
            }
        ...
        bool full() const { return size_ == RESP_UNIQUE_ARRAY_NODE_CAPACITY; }
  ...
  public:
    unique_array() : node_size_(0), size_(0) {}

    explicit unique_array(size_t capacity) : node_size_(0), size_(0) {
        reserve(capacity);
    }

    unique_array(unique_array const &other)
        : index_(other.index_), node_size_(other.node_size_),
          size_(other.size_) {
  ...
  public:
    ...
    void reserve(size_t capacity) {
        ...
        if (capacity > curr_capacity) {
            ...
            size_t incr_node_size =
                incr_size % node_capacity == 0 ? base_size : base_size + 1;
            grow(incr_node_size);
        }
    }
